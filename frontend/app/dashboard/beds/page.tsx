...
import { useHospital, useDepartment, useWard } from "@/lib/api/contexts";
...
function BedsContent() {
  ...
  const [maintenanceForm, setMaintenanceForm] = useState({
    bed: "",
    issue_description: "",
    maintenance_type: "repair" as "repair" | "preventive" | "inspection" | "replacement",
  ...
  const availableWards = useMemo(() => {
    if (draftFilters.department) {
      return wards.filter((w) => w.department === draftFilters.department);
    }
    return wards;
  }, [draftFilters.department, wards]);

  const selectedDepartmentWards = createDepartmentId
    ? wards.filter((w) => w.department === createDepartmentId)
    : [];
  ...
  // FIX: Removed selectedBed from dependencies and removed the selectedBed
  // refresh logic from inside this function. That logic caused a setState
  // → re-create callback → re-fire useEffect → setState loop.
  // Selected-bed detail is already refreshed explicitly inside runBedAction.
  const loadHospitalData = useCallback(async () => {
    if (!hospital) return;
    setIsBusy(true);
    setError(null);
    try {
      const [tagPage, bedPage, statBlock, analyticsBlock, maintenancePage, requestPage, admissionPage] =
        await Promise.all([
          bedService.getEquipmentTags({ is_active: true }),
          bedService.getBeds({ hospital: hospital.id, page_size: 100, ...appliedFilters }),
          bedService.getStatistics(hospital.id),
          bedService.getAnalytics(hospital.id),
          bedService.getMaintenanceRecords({ hospital: hospital.id, page_size: 100 }),
          admissionService.getAdmissionRequests({ hospital: hospital.id, page_size: 100 }),
          admissionService.getActiveAdmissions({ hospital: hospital.id, page: 1 }),
        ]);
  ...
  }, [appliedFilters, getErrorMessage, hospital]);
        ...
        {activeTab === "inventory" ? (
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
            <div className="grid gap-6">
              ...
              <Section
                title="Bed Inventory"
                description={`${beds.length} beds currently in view for ${hospital?.name || "this hospital"}.`}
              >
              ...
              <Field label="Department">
                <select
                  value={departments.find((department) =>
                    wards.filter((w) => w.department === department.id).some((ward) => ward.id === createForm.ward_id)
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label="Type">
                <select
                  ...
                  onChange={(event) =>
                    setMaintenanceForm((current) => ({ ...current, maintenance_type: event.target.value as "repair" | "preventive" | "inspection" | "replacement" }))
                  }
                  className="border border-slate-300 px-3 py-2 text-sm outline-none focus:border-emerald-600"
                >
              ...
              <Field label="Severity">
                <select
                  ...
                  onChange={(event) => setMaintenanceForm((current) => ({ ...current, severity: event.target.value as "low" | "medium" | "high" | "critical" }))}
                  className="border border-slate-300 px-3 py-2 text-sm outline-none focus:border-emerald-600"
                >
